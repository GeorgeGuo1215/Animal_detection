...
from .text_utils import chunk_text, cleanup_text, iter_txt_files, read_text_lossy
...
def build_or_update_index(
    ...
    *,
    ...
    files = list(iter_txt_files(cfg.raw_dir))
    ...
    for path in tqdm(files, desc="Indexing .txt", unit="book"):
...
def search(
    ...
    *,
    ...
    store = NumpyVectorStore(cfg.index_dir)
    if not store.exists():
        raise FileNotFoundError(f"Index not found: {cfg.index_dir} (run ingest first)")
    store.load()
    embedder = Embedder(cfg.embedding_model, device=device)
