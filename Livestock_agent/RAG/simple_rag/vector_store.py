...
class NumpyVectorStore:
    ...
    def config(self) -> StoreConfig:
        if self._cfg is None:
            raise RuntimeError("store config not loaded/initialized")
        return self._cfg
    ...
    def load(self) -> None:
        ...
        if self._emb.ndim != 2 or self._emb.shape[0] != len(self._meta):
            raise ValueError(f"embeddings/meta mismatch: emb={self._emb.shape} meta={len(self._meta)}")
        if self._emb.shape[1] != self.config.dim:
            raise ValueError(f"dim mismatch: cfg={self.config.dim} emb={self._emb.shape[1]}")
    ...
    def add(self, vectors: np.ndarray, metas: List[dict]) -> int:
        if self._cfg is None or self._emb is None:
            raise RuntimeError("store not initialized/loaded")
        if vectors.dtype != np.float32:
            vectors = vectors.astype(np.float32)
        if vectors.ndim != 2 or vectors.shape[0] != len(metas):
            raise ValueError("vectors/metas count mismatch")
        if vectors.shape[1] != self.config.dim:
            raise ValueError(f"dim mismatch: store={self.config.dim} vectors={vectors.shape[1]}")
    ...
    def search(self, query_vec: np.ndarray, top_k: int = 5) -> List[Tuple[dict, float]]:
        ...
        if q.ndim != 1 or q.shape[0] != self.config.dim:
            raise ValueError(f"query dim mismatch: expected {self.config.dim}, got {q.shape}")
        scores = self._emb @ q
        if self.size == 0:
            return []
