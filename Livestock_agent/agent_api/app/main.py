...
from .qa_store import (
    get_feedback_stats, get_knowledge_gaps, get_qa_stats, get_quality_report,
...
from .plan_and_solve import AsyncPlanAndSolveAgent, harden_plan_steps
from .trace_store import new_trace_id, write_trace


app = FastAPI(title="LivestockMind Agent API", version="1.0.0",
              description="Large Livestock Veterinary Expert System — RAG + Agent")
...
def _startup() -> None:
    ...
    if os.getenv("AGENT_WARMUP_RAG", "1") == "1":
        ...
        try:
            ...
            print(f"[startup] Index: {stats['index_size']} chunks from {cfg.index_dir}")
...
def health() -> Dict[str, Any]:
    return {"ok": True, "service": "LivestockMind"}
...
async def agent_plan_and_solve(req: AgentPlanAndSolveRequest) -> AgentPlanAndSolveResponse:
    ...
    try:
        ...
        plan = harden_plan_steps(plan, query=req.query, allowed_tools=req.allowed_tools)
...
async def get_session(session_id: str):
    ...
    return {"ok": True, "session_id": sess.session_id, "messages": sess.messages,
            "preferences": sess.preferences, "pending_clarification": sess.pending_clarification,
...
async def qa_history(
    ...
    user_role: Optional[str] = Query(None, pattern="^(farmer|veterinarian)?$"),
) -> Dict[str, Any]:
    data = await query_qa_history(
        page=page, page_size=page_size,
        date_from=date_from, date_to=date_to, keyword=keyword, user_role=user_role,
    )
    return {"ok": True, **data}
...
async def qa_stats(
    ...
    data = await get_qa_stats(date_from=date_from, date_to=date_to, user_role=user_role)
    return {"ok": True, **data}
...
@app.get("/qa/quality-report", dependencies=[Depends(_require_admin)])
async def qa_quality_report(
    date_from: Optional[str] = Query(None, description="YYYY-MM-DD"),
    date_to: Optional[str] = Query(None, description="YYYY-MM-DD"),
    user_role: Optional[str] = Query(None, pattern="^(farmer|veterinarian)?$"),
    limit: int = Query(200, ge=1, le=1000),
    sample_size: int = Query(30, ge=1, le=200),
) -> Dict[str, Any]:
    data = await get_quality_report(
        date_from=date_from, date_to=date_to, user_role=user_role, limit=limit, sample_size=sample_size,
    )
    return {"ok": True, **data}
...
async def qa_feedback(request: Request) -> Dict[str, Any]:
    ...
    status = await submit_feedback(request_id=request_id, rating=rating, comment=comment)
    if status == "not_found":
        raise HTTPException(status_code=404, detail="Record not found for this answer yet")
    if status == "already_rated":
        raise HTTPException(status_code=409, detail="This answer has already been rated")
    return {"ok": True}
...
async def qa_feedback_stats(
    ...
    data = await get_feedback_stats(date_from=date_from, date_to=date_to, user_role=user_role)
    return {"ok": True, **data}
