...
### n8n 设备事件入站（integration 包）

- **`POST /integration/ingest`**：转发 JSON 至 `N8N_WEBHOOK_URL`（实现见仓库根目录 `integration/`），**免 API Key**，便于浏览器 / 蓝牙上报。
- **`/integration/debug/*`**：联调 n8n，同样免 Key。
- 环境变量：`N8N_WEBHOOK_URL`、`N8N_TIMEOUT_SEC`（见 `agentAndRag/.env.example` 与 `integration/.env.example`）。
...
| `AGENT_ENABLE_CORS` | `1` | 默认开启，便于 `localhost:8001` 等前端访问 `8000`；设为 `0` 关闭 |
...
│   │   ├── routers/              # HTTP 路由
│   │   │   ├── routes_openai.py    # /v1/chat/completions (SSE)
│   │   │   └── routes_chat_ui.py   # /chat、管理页
│   │   ├── services/
│   │   │   └── plan_and_solve.py   # Plan-and-Solve Agent
│   │   ├── llm/
│   │   │   ├── llm_client.py
│   │   │   └── llm_client_stream.py
│   │   ├── tools/
│   │   │   ├── tool_registry.py
│   │   │   ├── tools_builtin.py
│   │   │   ├── tools_mcp.py
│   │   │   └── rag_tools.py        # RAG 检索桥接
│   │   ├── mcp/
│   │   │   ├── mcp_client.py
│   │   │   └── mcp_config.py
│   │   ├── persistence/
...
│   │   │   └── session_manager.py
│   │   ├── middleware/             # auth、限流
│   │   ├── context/                # 请求级 context（如 animal_id）
│   │   ├── schemas/                # Pydantic（tool_api + openai_schemas）
│   │   └── sql_search/             # sql.search 只读查询
